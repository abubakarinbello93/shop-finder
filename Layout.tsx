  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white border-b px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        <h1 className="text-blue-600 font-black text-xl tracking-tighter uppercase">SHOP FINDER</h1>
      ...
      `}>
        <div className="h-full flex flex-col">
          <div className="p-8 hidden md:flex items-center gap-3">
             ...
             <h1 className="text-blue-600 font-black text-2xl tracking-tighter uppercase">SHOP FINDER</h1>
          </div>
          ...
          <div className="px-8 py-2 mb-6">
            ...
            {shop && <p className="text-xs font-black text-blue-600 mt-1 uppercase truncate tracking-tight">{shop.name}</p>}
          </div>
          ...
          <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
            {menuItems.map((item) => (
              <Link
                ...
                `}
              >
                <item.icon className={`mr-3 h-5 w-5 ${location.pathname === item.path ? 'text-blue-600' : 'text-slate-400'}`} />
                {item.name}
              </Link>
            ))}
            ...
            {shop && (
              <div className="mt-8 pt-6 border-t border-slate-100 space-y-4 px-2">
                <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest px-2">Facility Manager</p>
                
                <button 
                  onClick={() => setShowCommentModal(true)}
                  className="w-full flex items-center px-4 py-3 text-sm font-black text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-100"
                >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 border-b bg-slate-50/50 flex justify-between items-center">
              <h2 className="text-xl font-black text-[#0f172a] tracking-tight uppercase">Status Update</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl flex items-start gap-3">
                 <Bell className="h-5 w-5 text-blue-600 shrink-0 mt-1" />
              ...
              <div className="relative">
                <textarea 
                  className="w-full p-4 bg-slate-50 border-2 border-transparent rounded-xl font-bold text-[#0f172a] focus:border-blue-600 focus:bg-white outline-none resize-none min-h-[140px] transition-all"
              ...
              <button 
                ...
                className="w-full py-4 bg-blue-600 text-white font-black rounded-xl shadow-xl shadow-blue-100 flex items-center justify-center gap-3 hover:bg-blue-700 transition-all disabled:opacity-50 disabled:shadow-none uppercase tracking-widest text-sm"
              >
                <Send className="h-5 w-5" /> Post Update
              </button>
            </div>
          </div>
        </div>
