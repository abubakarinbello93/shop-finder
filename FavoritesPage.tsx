      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
        {favoriteShops.map(shop => (
          <div 
            ...
            className="bg-white rounded-[24px] md:rounded-3xl shadow-sm border p-3 md:p-6 flex flex-col gap-2 md:gap-4 relative hover:shadow-md transition-shadow cursor-pointer group"
          >
            <button 
              onClick={(e) => { e.stopPropagation(); onToggleFavorite(shop.id); }}
              className="absolute top-2 right-2 md:top-4 md:right-4 p-1.5 md:p-2 text-red-500 bg-red-50 rounded-full z-10"
            >
              <Heart className="h-4 w-4 md:h-5 md:w-5 fill-current" />
            </button>
            <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
              <div className="p-2 md:p-3 bg-gray-100 rounded-xl md:rounded-2xl group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors shrink-0">
                <Store className="h-5 w-5 md:h-7 md:w-7" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="font-black text-gray-900 text-xs md:text-xl leading-tight truncate">{shop.name}</h3>
                <p className="text-[8px] md:text-sm font-bold text-gray-400 uppercase tracking-widest truncate">{shop.code}</p>
              </div>
            </div>
            <div className="flex items-center justify-between mt-auto pt-2 md:pt-4 border-t border-gray-50">
              <div className="flex items-center gap-1.5">
                <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${shop.isOpen ? 'bg-blue-600' : 'bg-red-600'}`}></div>
                <span className={`text-[8px] md:text-sm font-black uppercase tracking-wider ${shop.isOpen ? 'text-blue-600' : 'text-red-600'}`}>
                  {shop.isOpen ? 'OPEN' : 'CLOSED'}
                </span>
              </div>
              <ChevronRight className="h-3 w-3 md:h-4 md:w-4 text-blue-600 hidden xs:block" />
            </div>
          </div>
        ))}

        {favoriteShops.length === 0 && (
          <div className="col-span-full py-16 md:py-32 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
            <Heart className="h-10 w-10 md:h-16 md:w-16 text-gray-200 mx-auto mb-4" />
            <p className="font-black text-gray-400 text-sm md:text-xl">No favorites found.</p>
          </div>
        )}
      </div>
